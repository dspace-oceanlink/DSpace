...
public class DSpaceBitstreamAdapter extends AbstractDSpaceEventAdapter {
    ...
    protected Resource create(DSpaceObject subject) throws RDFHandlerException, RepositoryException {
        ...
        handleStatement(r, RDF.TYPE, DS.Bitstream);
        return r;
    }
    ...
    protected void modifyMetadata(Context ctx, DSpaceObject subject) throws Exception {
        ...
        */
        ...
        {
            this.getProvider().getConnection().add(valueFactory.createStatement(uri, DC.title_,
                    valueFactory.createLiteral(StringEscapeUtils
                            .escapeXml(bitstream.getName()))),DS.DescriptiveMetadata);
        }

        if(bitstream.getDescription() != null)
        {
            this.getProvider().getConnection().add(valueFactory.createStatement(uri, DC.description_,
                    valueFactory.createLiteral(StringEscapeUtils
                            .escapeXml(bitstream.getDescription()))),DS.DescriptiveMetadata);
        }

        this.getProvider().getConnection().add(valueFactory.createStatement(uri, DS.size,
                valueFactory.createLiteral(bitstream.getSize())),DS.DescriptiveMetadata);

        this.getProvider().getConnection().add(valueFactory.createStatement(uri, DS.messageDigest,
                valueFactory.createLiteral(StringEscapeUtils
                        .escapeXml(bitstream.getChecksum()))),DS.DescriptiveMetadata);

        this.getProvider().getConnection().add(valueFactory.createStatement(uri, DS.messageDigestAlgorithm,
                valueFactory.createLiteral(StringEscapeUtils
                        .escapeXml(bitstream.getChecksumAlgorithm()))),DS.DescriptiveMetadata);
        ...
        for (String bundleName : bundleSet) {
            this.getProvider().getConnection().add(valueFactory.createStatement(uri,
                    DS.bundle, valueFactory
                    .createLiteral(StringEscapeUtils
                            .escapeXml(bundleName))),DS.DescriptiveMetadata);
        }
        ...
        this.getProvider().getConnection().add(valueFactory.createStatement(uri,
                DCTERMS.format_, fu),DS.DescriptiveMetadata);

        this.getProvider().getConnection().add(valueFactory.createStatement(fu,
                RDF.TYPE, DS.BitstreamFormat),DS.DescriptiveMetadata);
        ...
        this.getProvider().getConnection().add(valueFactory.createStatement(fu,
                ...
                .createLiteral(supportLevel)),DS.DescriptiveMetadata);
